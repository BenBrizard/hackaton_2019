        ...
        for j in range(0,3*array_length) :
            ...
            if (i >= 0 and i < centerDown) :
                ...
                if(masque_tot[i,j]==True):
                    ...
                    data[i,j,0]=abs(v1[j]+v2[j])
                    data[i,j,1]=abs(v1[j]+v2[j])
                    data[i,j,2]=abs(v1[j]+v2[j]) 
                else:
            ...
            #ligne du centre
                ...
                if (j >=0 and j<=array_length) :
                 ...
                 # colonnes du centre
                    ...
                    minimum = np.min(abs(angle[i,j]-angle_array));
                    index_angle = np.where(abs(angle[i,j]-angle_array) == minimum)[0];
                    data[i,j,0]=abs(v1[index_angle+500]+v2[index_angle+500])
                    data[i,j,1]=abs(v1[index_angle+500]+v2[index_angle+500])
                    data[i,j,2]=abs(v1[index_angle+500]+v2[index_angle+500])
                elif(masque_tot[i,j]==True and i<center):
